...
### 缺陷修复原则
- 修复 bug 时，禁止采用临时性补丁、表层兜底或仅为恢复现象的最小改动方案。
- 必须先定位根因，再基于完整调用链、数据流和边界条件设计正式修复方案。
- 修复实现应优先消除错误成因，而不是仅掩盖异常表现、绕过失败分支或追加脆弱判断。
- 缺陷处理需遵循当前项目分层约束、可维护性要求和通用最佳实践，避免引入新的耦合、重复逻辑或隐性技术债务。
- 涉及核心流程、聚合逻辑、外部客户端适配和异常链路的 bug，修复时应同步评估相关测试、配置和兼容性影响，确保方案长期稳定可维护。
...
- 后续新增或更新的设计稿（spec）和计划稿（plan）统一使用中文撰写。
- 设计稿默认写入 `docs/superpowers/specs/`，计划稿默认写入 `docs/superpowers/plans/`。
- 文档中的接口名、类名、配置键、命令行等技术标识保持原始大小写，可配合中文说明，但不要整体翻译为中文。
- 除非用户明确要求英文版，否则不要输出英文设计稿或英文计划稿。

**关键**：文档语言统一是项目协作规则的一部分，后续产出的 spec 与 plan 必须默认遵循中文规范。
