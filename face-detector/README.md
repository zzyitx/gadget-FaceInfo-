...
检测后的候选框去重策略也参考了 CompreFace 的 `similarity threshold` 思路：

- 优先对空间上疑似重复的人脸框计算轻量特征相似度
- 相似度会被映射到 `0..1` 区间，再按阈值判定是否视为同一张脸
- 当相似度计算异常时，自动降级回原有 `IoU` 去重逻辑

这样可以优先减少同一张脸被多个后端重复框选的问题，同时保留旧方案作为稳定兜底。
