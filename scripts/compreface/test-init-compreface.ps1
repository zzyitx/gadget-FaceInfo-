...
Assert-Contains -Pattern '\$BaseUrl/admin/app' -Message "script should query existing applications before creating anything"
...
if ($scriptContent -match 'Write-Host "Creating application: \$ApplicationName"' -or
    $scriptContent -match 'Invoke-CompreFaceJson\s+-Method Post\s+-Uri "\$BaseUrl/admin/app"') {
    throw "script should not create gadget application when it already exists"
}

Write-Host "PASS: init-compreface script contains reuse-existing-app logic."
